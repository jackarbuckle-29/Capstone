...
# XGBoost Model Tuning - Men's Lacrosse
...
save(test, file = "lax0_run_test.rda")

high_speed_dist_var_name <- "high_speed_distance"
lax0_data <- read.csv("lax0_data_cleaned.csv") 
...
selected_sport <- "Men's Lacrosse"
xgb_viz_title <- "XGBoost Model Actual vs Predicted High Speed Distance"
...
save(prelim_importance, bst_split_pred_data_pre_tune, 
     pred_viz_pre_tune, diff_hist_pre_tune, file = "lax0_pre_tune_files.rda")
...
save(res_db, bst_max_depth, bst_min_child, g_2, 
     file = "lax0_min_child_max_depth.rda")
...
save(res_db, bst_col_samp, bst_sub_samp, g_4, 
     file = "lax0_sample_colsample.rda")
...
lax0_full_preds_df <- rbind(bst_split_pred_data_pre_tune, bst_pred_data)
