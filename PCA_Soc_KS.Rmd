...
```{r}
file_path <- getwd()
soc0_data <- read.csv(paste0(file_path, "\\Exported_CSVs\\soc0_data_cleaned.csv")) 
soc1_data <- read.csv(paste0(file_path, "\\Exported_CSVs\\soc1_data_cleaned.csv"))  
```
...
dim(comb_soc)
# setdiff(names(comb_soc), names(model_df))
```
...
model_df <- comb_soc %>% 
  ...
  select(!"high_speed_distance_z") %>%
  filter(high_speed_distance >0)

model_df <- model_df[ , which(apply(model_df, 2, var) != 0)]

set.seed(33) # Set Seed
split_ratio <- 0.7
train_row_ind <- sample(x=nrow(model_df), size=floor(nrow(model_df) * split_ratio))
...
```{r}
options(scipen=999)
pcs <- prcomp(train_data, scale.=TRUE)
#summary(pcs)

vars <- apply(pcs$x, 2, var)  
props <- vars / sum(vars)
pc_co_num <- min(which(cumsum(props) > 0.80))

pcs$rotation[,1:34]
pcs$center
pcs$x[1:10,1:34]

data1 <- data.frame(cbind(train_data$high_speed_distance, pcs$x[,1:34]))
names(data1)[1] <- "high_speed_distance"
head(data1)

# length(train_data$high_speed_distance)
# row.names(model_df)
# nrow(pcs$x[,1:34])


model <- lm(high_speed_distance ~ ., data = data1)
summary(model)


test_pca_pred <- predict(pcs, newdata=test_data)
predictions <- predict(model, data.frame(test_pca_pred))

comp_df <- data.frame(actual = test_data$high_speed_distance,
                       estimated = predictions,
                       residuals = test_data$high_speed_distance - predictions)
...
```{r}
dim(comp_df[abs(comp_df$residuals) <= 150, ])[1]/(dim(comp_df)[1])
summary(comp_df$residuals)

wi_cutoff <- comp_df %>%
  filter(abs(residuals) <= 150) %>% nrow()
wi_cutoff / nrow(comp_df)
```
...
data1 <- data.frame(cbind(train_data$high_speed_distance, pcs$x[,1:29]))
head(data1)
names(data1)[1] <- "high_speed_distance"
model <- lm(high_speed_distance ~ ., data = data1)
summary(model)
...
test_pca_pred_2 <- predict(pcs, newdata=test_data)
predictions <- predict(model, data.frame(test_pca_pred_2))

comp_df_2 <- data.frame(actual = test_data$high_speed_distance,
                         estimated = predictions,
                         residuals = test_data$high_speed_distance - predictions)
comp_df_2
...
```{r}
dim(comp_df_2[abs(comp_df_2$residuals) <= 150, ])[1]/(dim(comp_df_2)[1])
summary(comp_df_2$residuals)
...
scaled_data <- scale(model_df[,-1])
scaled_data <- data.frame(test111)
scaled_data <- test111[ , which(apply(test111, 2, var) != 0)]

test111[train_row_ind, ]
...
data1_test <- data.frame(cbind(model_df$high_speed_distance, pcs_test$x[,1:29], test111)) %>%
...
set.seed(33) # Set Seed
split_ratio <- 0.7
train_row_ind <- sample(x=nrow(data1_test), size=floor(nrow(data1_test) * split_ratio))

# Split the data into training and testing sets
train_data <- data1_test[train_row_ind, ]
test_data <- data1_test[-train_row_ind, ]
...
#response <- data1_test$model_df.high_speed_distance
...
```{r}
everything_model <- lm(model_df.high_speed_distance ~ ., data = train_data)
predictions_test <- predict(everything_model, test_data[, -1])

valid.df <- data.frame(actual = test_data$model_df.high_speed_distance,
                       estimated = predictions_test,
                       residuals = test_data$model_df.high_speed_distance - predictions_test)
valid.df

train_data  %>% select(!contains(c("ima", "PC", "player_load"))) %>% names()
```
