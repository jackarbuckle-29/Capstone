...
```
```{r fig.width=6}
plot_data <- lax1_data %>% filter(high_speed_distance > 0) %>%
  select(high_speed_distance, player_load_2d, activity_type) 

plot_data %>%
  ggplot(aes(x=player_load_2d, y = high_speed_distance)) +
  geom_point(color = ifelse(plot_data$activity_type == "game", nd_green, nd_navy)) +
  theme_minimal() + xlim(0, 1000) +
  geom_smooth(color = nd_gold) + ylab("") + xlab("")

# + # Add smoothing line +
#   labs(title = "Analyzing Player Load 2D vs High Speed Distance",
#     subtitle = "Outdoor Data, Lax1",
...
# write.csv(comb_z_df_full,"G:\\Shared drives\\Analytics Capstone\\R Project - Analytics Capstone\\Exported_CSVs\\lax1_data_cleaned_with_z.csv",row.names=FALSE) 
```
...
plot_vars_lax1 <- c(high_speed_dist_var_name,
...
lax1_2d_game_plot <- ggplot(data = lax1_plot_game_df,
  ...
  geom_smooth(color = nd_gold) + # Add smoothing line +
  labs(title = "Analyzing Player Load 2D vs High Speed Distance",
    subtitle = "Game Only, Outdoor Data, Lax1",
     x = "Player Load 2D",y = "High Speed Distance")
...
lax1_2d_train_plot <- ggplot(data = lax1_plot_training_df,
  ...
  geom_smooth(color = nd_gold) + # Add smoothing line +
  labs(title = "Analyzing Player Load 2D vs High Speed Distance",
    subtitle = "Training Only, Outdoor Data, Lax1",
     x = "Player Load 2D",y = "High Speed Distance")
...
```{r NEW training test data - reg and z-score}
...
bst_split_mod_pre_tune_lax1 <- xgboost(
    data = dtrain,  # Exclude the response variable
    label = train_data$high_speed_running_distance_session,
...
bst_preds_pre_tune <- predict(bst_split_mod_pre_tune_lax1, as.matrix(test_data[, -1]))
bst_actual_pre_tune<- test_data$high_speed_running_distance_session
  
# Calculate RMSE (Root Mean Squared Error) for model evaluation
rmse <- sqrt(mean((bst_preds_pre_tune - test_data$high_speed_running_distance_session)^2))
...
importance <- xgb.importance(feature_names = colnames(as.matrix(train_data[, -1])),
                             model = bst_split_mod_pre_tune_lax1)
...
mod_pred_viz_pre_tune_lax1 <- ggplot(pred_data, aes(x = predicted, y = actual)) +
  geom_point(color=nd_navy) +
...
viz_time <- "pre-tuning"
pre_tune_diff_hist <- ggplot(bst_pred_data,
...
for (r in 1:length(split_ratio_vec)){
  ...
  test_y = test_data[,1]
  test_x = test_data[,-1]
  response <- model_df$high_speed_running_distance_session
  ...
  actual<- test_data$high_speed_running_distance_session
  
  # Calculate RMSE (Root Mean Squared Error) for model evaluation
  rmse <- sqrt(mean((predictions - test_data$high_speed_running_distance_session)^2))
...
bst_final_actual<- test_data$high_speed_running_distance_session
  
# Calculate RMSE (Root Mean Squared Error) for model evaluation
rmse <- sqrt(mean((bst_final_preds - test_data$high_speed_running_distance_session)^2))
#You can also inspect the model's feature importance if needed
importance <- xgb.importance(feature_names = colnames(as.matrix(train_data[, -1])),
                             model = bst_final_mod)
...
model_df <- z_score_df[,-1]  # dropping unique session
...
xgb_model <- xgboost(
  data = as.matrix(train_data[, -1]),  # Exclude the response variable
  label = train_data$high_speed_running_distance_session_z,
  booster = "gblinear",  # Use linear booster for regression
  objective = "reg:linear",  # Specify regression as the objective
  eval_metric = "rmse",  # Evaluation metric (Root Mean Squared Error)
  nrounds = 500,  # Number of boosting rounds (you can adjust this)
...
actual<- test_data$high_speed_running_distance_session_z

# Calculate RMSE (Root Mean Squared Error) for model evaluation
rmse <- sqrt(mean((predictions - test_data$high_speed_running_distance_session_z)^2))
cat("Root Mean Squared Error (RMSE):", rmse, "\n")
...
model_df <- sub_model_df[,-1] %>% 
  filter(high_speed_running_distance_session >= 100)
...
model_df <- z_score_df[,-1] %>% 
  filter(high_speed_running_distance_session_z >= 100)
...
test_y = test_data[,1]
test_x = test_data[,-1]
response <- model_df$high_speed_running_distance_session_z
...
```{r partition data}
#lax1_mod_df <- sub_model_df

set.seed(33) # Set Seed

# Split the data into training and testing sets
split_ratio <- 0.60 # 70% training data, 30% testing data
split_index <- floor(nrow(lax1_mod_df) * split_ratio)
train_data <- lax1_mod_df[1:split_index, ]
test_data <- lax1_mod_df[(split_index + 1):nrow(lax1_mod_df), ]
...
response <- lax1_mod_df$high_speed_running_distance_session
...
lm_all <- lm(high_speed_running_distance_session ~ .,data = train_data, na.action = na.exclude)
summary(lm_all)
```
 
#### <span style= "color:#AE9142" > Forward Selection Method </span>
```{r forward, results='hide'}
lm_null <- lm(high_speed_running_distance_session~1, data = train_data, na.action=na.exclude)
...
fwd_acc <- forecast::accuracy(lm_fwd_pred,
                              test_data$high_speed_running_distance_session) #accuracy
```
...
back_acc <- forecast::accuracy(lm_back_pred,
                               test_data$high_speed_running_distance_session)
```
...
for (i in 1:length(mod_list)) {
  lm_mod <- lm(mod_list[[i]]$terms, lax1_mod_df)
